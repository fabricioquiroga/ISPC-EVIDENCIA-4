...
class TestExtrusoraDeCarne(unittest.TestCase):
    ...
    def test_agregar_carne(self):
        extrusora = ExtrusoraDeCarne(carne_inicial=100)  
        cantidad_a_agregar = random.randint(1, 50)
        extrusora.agregar_carne(cantidad_a_agregar)
        self.assertEqual(extrusora.obtener_cantidad_carne(), 100 + cantidad_a_agregar)
    
    def test_procesar_carne(self):
        extrusora = ExtrusoraDeCarne(carne_inicial=100)  
        cantidad_a_procesar = random.randint(1, 50)
        extrusora.procesar_carne(cantidad_a_procesar)
        self.assertEqual(extrusora.obtener_cantidad_carne(), 100 - cantidad_a_procesar)
